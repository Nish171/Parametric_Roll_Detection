...
class Dataset:
    def __init__(self, input_dim, pred_dim, shift, skip=1, hop=0.25, batch_size=1, normalizer=1, classification=False, in_cols=['roll'], out_cols=['roll']):
        ...
        self.sim_folder_name                = "RegWave_Simulations1"
        self.num_sims                       = 110
    ...
    def _data_stats_(self):
        ...
        for i in range(1, self.num_sims+1):
            ...
            lam             = 9.81*float(Tp)*float(Tp)/(2*np.pi)
            # lam = 1
            stats           = stats.append({'Sim_no': i, 'fac': round(lam/float(Hs),0), 'Hs':Hs, 'Tp':Tp, 'Max_roll':max_roll}, ignore_index=True)
        return stats

    def stats_table(self):
        stats = self._data_stats_()
        cols = stats['Tp'].unique()
        rows = stats['fac'].unique()
        stats_df = pd.DataFrame(index=rows, columns=cols)
        for tp in cols:
            for hs in rows:
                hs_df = stats[stats['fac']==hs]
